...
public class ResourceManager : MonoBehaviour
{
    ...
    {
        GameObject prefabMap = await LoadAsset<GameObject>(address);

        if (prefabMap == null)
        {
            Debug.LogWarning($"[ResourceManager] 프리팹을 로드하지 못해 생성할 수 없습니다. {address}");
            return null;
        }

        GameObject instance = Object.Instantiate(prefabMap, parent, instantiateInWorldSpace);
